...
function colorFromValue(value, border) {
  ...
  const tgColor = 'purple'
  if (border) {
    alpha += 0.01
  }
  return color(tgColor).alpha(alpha).rgbString()
}
...
onMounted(() => {
  const ctx2d = chartArea.value.getContext('2d')
  window.myTreeMap = new Chart(ctx2d, config)
})
</script>
<template>
  <div id="canvas-holder">
    <canvas id="chart-area" ref="chartArea" width="800" height="400"></canvas>
  </div>
</template>
